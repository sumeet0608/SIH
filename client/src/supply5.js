...
function Supply() {
    ...
    const handlerSubmitSold = async (event) => {
        event.preventDefault();
        try {
            var reciept = await SupplyChain.methods.sold(ID,Quantity).send({ from: currentaccount });
            if (reciept) {
                loadBlockchaindata();
            }
        }
    ...
    return (
        <div>
            ...
            <form style={{paddingLeft : 60}} onSubmit={handlerSubmitSold}>
                <br></br><br></br>
             
             <input style={{marginLeft : 40,width:'20em',height:'3em'}} className="form-control-sm" type="text" onChange={handlerChangeID} placeholder="Enter Patient Name" required />
             <input style={{marginLeft : 40,width:'20em',height:'3em'}} className="form-control-sm" type="email" onChange={handlerChangeQuantity} placeholder="Enter Patient email" required />
                <br></br>
                
            <button style={{marginLeft : 40,width:'10em',backgroundColor : 'whitesmoke',borderRadius : '2em',color:'black',fontSize : 'large'}} className="btn btn-outline-success btn-sm" onSubmit={handlerSubmitSold}>Supply</button>
