...
function Supply() {
    ...
    const handlerSubmitManufacturing = async (event) => {
        event.preventDefault();
        try {
            var reciept = await SupplyChain.methods.Manufacturing(ID,Quantity).send({ from: currentaccount });
            if (reciept) {
                loadBlockchaindata();
            }
        }
    ...
    return (
        <div>
            ...
            <form style={{paddingLeft : 60}} onSubmit={handlerSubmitManufacturing}>
                <br></br><br></br>
            ...
            <button style={{marginLeft : 40,width:'10em',backgroundColor : 'whitesmoke',borderRadius : '2em',color:'black',fontSize : 'large'}} className="btn btn-outline-success btn-sm" onSubmit={handlerSubmitManufacturing}>Manufacture</button>
